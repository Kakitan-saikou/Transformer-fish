...
print("local:", paras)
wandb.init(project="foil_search", config=paras, name=args.name)
...
if 'ip' in paras.__dict__:
    ...
    env = foil_env(paras, info=f"{dir_name}", local_port=None)
...
for i in tqdm(range(paras.epochs)):
    ...
    while not done:
        action = agent.choose_action(time_cost, dt=info['dt'])
    ...
    with open(r'./search.csv', mode='a', encoding='utf8') as cfa:
        wf = csv.writer(cfa)
        wf.writerow([paras.St, paras.AD, paras.theta, paras.Phi, ct_avg, cp_avg,-1* eta_avg, epsoide_length, Gt, avg_reward, cp_avg, fx_avg, np.sum(dt_ls)])
    Gt = 0
    epsoide_length = 0
    epsoide_num += 1
