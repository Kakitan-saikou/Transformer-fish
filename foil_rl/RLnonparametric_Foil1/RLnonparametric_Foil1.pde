...
float nextactionAoA=0,nextactionA=0,nextactionAvel = 0, nextactionAoAvel = 0;
float formerA = 0, formerAoA = 0,formerAvel = 0, formerAoAvel = 0;
float[] NextAction = {nextactionA,nextactionAvel,nextactionAoA, nextactionAoAvel};
...
void draw(){
  if (test.t <= EpisodeTime){
    ...
    if(reset_flag == 1){
     ...
     String state_cn = multy_state(y,angle,0,0,0,0,Sp,test.flow.dt/32.,velvalue);
     state.clear();
     state.add(state_cn);
    }
    ...
    if (test.t<ActionTime){
      ...
      String state_cn = multy_state(y,angle,velocity,velocitx,velocityphi,Eta,Sp,test.flow.dt/32.,velvalue);
      state.clear();
      state.add(state_cn);
    ...
    */
    ...
    float M = test.foil.pressMoment(test.flow.p);
    forceX = - forces.x;
    // used for vel sensing
    float py = coordinate.y-32;
    float px = coordinate.x-72;
    velvalue = addVelField(test.flow.u, px, py);
    ...
    String state_cn = multy_state(y,angle,velocitx,velocity,velocityphi,Eta,Sp,test.flow.dt/32.,velvalue);
    state.clear();
    state.add(state_cn);
    ...
    dat.output.println(test.t + " " + forces.x + " " + forces.y + " " + angle + " " + test.foil.xc.y  + " " + velocitx + " " + velocity + " " + velocityphi + " " +CT + " " + CP + " " + Eta + " " + Sp + ";");
    CT = 0;
...
public String multy_state(float py,float ptheta,float yvel,float xvel,float tvel,float peta,String Sp,float ddt,ArrayList<PVector> velfield) {
  ...
  multy_state_json.put("dt", ddt);
  multy_state_json.put("Velfield", velfield);
...
//vel sensing
ArrayList<PVector> addVelField(VectorField u, float px, float py) {
    ArrayList<PVector> velsenpoints = new ArrayList<PVector>();
    PVector temppoint = new PVector(0,0);
    for (int j=round(py); j<=(py+64); j++) {
      for (int i=round(px); i<=(px+80); i++) {
        float tempx = u.x.a[i][j];
        float tempy = u.y.a[i][j];
        temppoint.x = tempx;
        temppoint.y = tempy;
        velsenpoints.add(temppoint);
      }
    }
    return velsenpoints;
  }
...
String SparsePressure(RotatingFoil test)
{
  ...
  for (int i = 0; i < (test.foil.coords.size()) / 10; i++)
  {
    Pressure = append(Pressure, test.flow.p.linear(test.foil.coords.get(i * 10).x, test.foil.coords.get(i * 10).y));
  }
