...
class Body {
  ...
  PVector velocity_new(float dt, float x, float y){
  ...
  boolean unsteady(boolean flag){return flag;}
  ...
  void rotate_xRandom( float dphi, PVector xR) {///Tree
    this.dphi = dphi;
    phi = phi+dphi;
    float sa = sin(dphi), ca = cos(dphi);
    for ( PVector x: coords ) rotate( x, xR, sa, ca ); 
    getOrth(); // get new orthogonal projection
    if (n>4) box.rotate(dphi);
  }  
  void rotate_toPhi_xRandom( float phinew, PVector xR) {///Tree
    this.dphi = phinew-phi;
    phi = phinew;
    float sa = sin(dphi), ca = cos(dphi);
    for ( PVector x: coords ) rotate( x, xR, sa, ca ); 
    getOrth(); // get new orthogonal projection
  ...
  void rotate_toPhi( float phinew){
    rotate_toPhi_xRandom(phinew, xc);
  }
  ...
  float pressPower_old ( Field p, float dt ) {//official
    float power = 0;
    for ( OrthoNormal o: orth ) {
  ...
  float pressPower ( Field p, float dt ) {//newly modified by TreeSun
    float power = 0;
    for ( OrthoNormal o: orth ) {
