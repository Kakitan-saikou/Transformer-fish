from env.flow_field_env import fake_env, foil_env, fish_env
import argparse
import json
...
def prepare_arguments():
    ...
    args.name = f"[Debug]foil_newEnvV7_PPO_transformer_{args.seed}"
...
wandb.init(project="Fish_0816", entity="krhkk", config=paras, name=args.name, mode="online")# ("disabled" or "online")
...
env = fish_env(paras, local_port=8686)
# obs = env.reset()
...
agent = TPPO(paras)
agent.load('C:/Users/LENOVO/Desktop/foil_rl/actor_0.pth')
...
for i in tqdm(range(config['epochs'])):
    ...
    info = [{}]
    info[0]['dt'] = 0
    # while epsoide_length < 800 and not any(done):
    while epsoide_length < 800:
        if epsoide_num == 0:
    ...
    # eta_avg = np.average(fx_ls[int(epsoide_length / 2):], weights=dt_ls[int(epsoide_length / 2):]) / np.average(cp_ls[int(epsoide_length / 2):], weights=dt_ls[int(epsoide_length / 2):])
