...
paras.device = "cuda"
### start env
env = fish_env(paras, local_port=8888)
...
agent = TPPO(paras)
agent.load('C:/Users/LENOVO/Desktop/foil_rl/actor_500.pth')#'/mnt/nasdata/runji/LilyPad/models/foil/rsac/run197/trained_model/actor_offline.pth')
# imitation learning
#agent.imitation_train(offline_data, iter=1500)
# save agent model
agent.save(run_dir, 'offline')

t_agent = parametric_agent(paras)
...
for i in tqdm(range(1)):
    ...
    eta_avg = np.average(fx_ls[int(epsoide_length / 2):], weights=dt_ls[int(epsoide_length / 2):]) / np.average(
